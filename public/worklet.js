...
var p = class {
	constructor(e) {
		...
		this.process = (e) => {
			...
			for (this.fillInputBuffer(e); this.hasEnoughDataForFrame(); ) {
				const r = this.generateOutputFrame();
				t.push(r);
			}
			return t;
		};
	...
	*stream(e) {
	...
	generateOutputFrame() {
		let e = new Float32Array(this.options.targetFrameSize),
			t = 0,
			r = 0;
...
var m = class extends AudioWorkletProcessor {
	...
	process(t, r, i) {
		...
		if (this._initialized && s instanceof Float32Array) {
			const n = this.resampler.process(s);
			for (const h of n)
				this.port.postMessage({ message: "AUDIO_FRAME", data: h.buffer }, [
					h.buffer,
				]);
		}
		return !0;
